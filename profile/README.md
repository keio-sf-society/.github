...
> [!IMPORTANT]
> PCのブラウザで作業してください。また、適宜リンク先に跳んで説明を確認してください。

GitHub アカウントを登録して、当 Organization に招待されることで、会誌の編集フォルダ、会計情報を集めたフォルダ、等々当会の全ての情報にアクセスできます。アクセス権を要するSF会員は以下の手順を実行してください。
...
- [github.com](https://github.com/) にアクセスし、サインアップからメールアドレスとパスワードを登録する。メールアドレスは自分が一番よく使うものがよいです。[アカウントの作成方法](https://docs.github.com/ja/get-started/start-your-journey/creating-an-account-on-github)
- 登録後は[二段階認証](https://qiita.com/kanamekun/items/c307c942508e9d64c35b)も設定してください。二段階認証設定時には、スマホなどの紛失に備えてリカバリーコードをダウンロードしてなくさないところに保存しておく。



登録が完了したら、登録したメールアドレスを管理者（役員）に伝えてください。(いまのところは「おくのたかし」へ。夏頃までには現役の方ができるようにします)

### 2. 招待の承認

<!-- 二つの方法があります。（あとで方法２に統合予定）
...
- **メールを確認する**
    件名が `[GitHub] @(ユーザー名) has invited you to join the @慶應義塾大学SF研究会 organization` というメールを探します。
- **ボタンをクリックする**：
    本文内にある **[Join @慶應義塾大学SF研究会]** という緑色のボタンをクリックします。
- **ブラウザで承認する**
    GitHub のサイトにリダイレクトされます。画面上の **[Join @慶應義塾大学SF研究会]** ボタンをクリックすれば完了です。
...
管理者が招待を完了するまでお待ちください。招待が完了したら、管理者から連絡がいく、もしくは招待用のメールが届きます。そうしたら、以下の手順に従って、参加を完了してください。

- [招待用ページ](https://github.com/orgs/keio-sf-society/invitation)へアクセスする。
- 画面に招待の詳細が表示されるので、**[Join @慶應義塾大学SF研究会]** を選択します。

> [!NOTE]
> * **有効期限：** 招待の有効期限は **7 日間** です。期限が切れた場合は、管理者に再送を依頼する必要があります。
...
[Overview for members](https://github.com/keio-sf-society?view_as=member) を参照してください。
